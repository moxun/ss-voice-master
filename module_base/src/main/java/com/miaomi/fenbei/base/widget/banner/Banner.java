package com.miaomi.fenbei.base.widget.banner;
...
import com.miaomi.fenbei.base.widget.banner.listener.OnBannerClickListener;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    public String tag = "common_banner";
    ...
    private int mIndicatorSelectedResId = R.drawable.common_gray_radius;
    private int mIndicatorUnselectedResId = R.drawable.common_white_radius;
    private int bannerPlaceholder = -1;
    private int mLayoutResId = R.layout.common_banner;
    ...
    private OnBannerChangeLisetner onBannerChangeLisetner;
...
//        dm = context.getResources().getDisplayMetrics();
        indicatorSize = DensityUtil.INSTANCE.dp2px(context,4f);
        initView(context, attrs);
    }

    public void setOnBannerChangeLisetner(OnBannerChangeLisetner onBannerChangeLisetner){
        this.onBannerChangeLisetner = onBannerChangeLisetner;
    }

    public interface OnBannerChangeLisetner{
        void onChange();
    }
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        mIndicatorSelectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_selected, R.drawable.common_gray_radius);
        mIndicatorUnselectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_unselected, R.drawable.common_white_radius);
        bannerPlaceholder = typedArray.getResourceId(R.styleable.Banner_banner_placeholder,-1);
        ...
        bannerBackgroundImage = typedArray.getResourceId(R.styleable.Banner_banner_default_image, R.drawable.common_default);
        typedArray.recycle();
    }
...
//    }
    ...
    public Banner setImages(List<BannerBean> imageUrls) {
        this.imageUrls = imageUrls;
        this.count = imageUrls.size();
        return this;
    }

    public void update(List<BannerBean> imageUrls, List<String> titles) {
        this.titles.clear();
        this.titles.addAll(titles);
        update(imageUrls);
    }

    public void update(List<BannerBean> imageUrls) {
...
//    public void updateOpenChatBannerData(BannerBean item) {
//        boolean isHave = false;
//        for (int i = 0; i < imageUrls.size(); i++) {
//            if (imageUrls.get(i).getMold() == item.getMold()) {
//                isHave = true;
//                imageUrls.set(i, item);
//                break;
//            }
//        }
//        if (!isHave) {
...
//                if (view.getTag() == REFRESH_TAG) {
//                    ProgressBar diamondsPb = view.findViewById(R.id.diamonds_pb);
//                    ProgressBar sharePb = view.findViewById(R.id.share_pb);
//                    ProgressBar attentionPb = view.findViewById(R.id.attention_pb);
//                    diamondsPb.setMax(Integer.parseInt(item.getPrice().substring(item.getPrice().indexOf("/") + 1, item.getPrice().length() - 1)) * 10000);
//                    diamondsPb.setProgress(Integer.parseInt(item.getPrice().substring(0, item.getPrice().indexOf("/"))));
//                    sharePb.setMax(Integer.parseInt(item.getShare_num().substring(item.getShare_num().indexOf("/") + 1)));
//                    sharePb.setProgress(Integer.parseInt(item.getShare_num().substring(0, item.getShare_num().indexOf("/"))));
//                    attentionPb.setMax(Integer.parseInt(item.getFollow_num().substring(item.getFollow_num().indexOf("/") + 1)));
//                    attentionPb.setProgress(Integer.parseInt(item.getFollow_num().substring(0, item.getFollow_num().indexOf("/"))));
//                    ((TextView) view.findViewById(R.id.diamonds_tv)).setText(item.getPrice());
//                    ((TextView) view.findViewById(R.id.share_tv)).setText(item.getShare_num());
//                    ((TextView) view.findViewById(R.id.attention_tv)).setText(item.getFollow_num());
//                }
//            }
...
//    }
    ...
    private void setImageList(List<BannerBean> imagesUrl) {
...
//            }
            ...
            ImgUtil.INSTANCE.loadRoundImg(context,url.getCover(), imageView,6, bannerPlaceholder);
    ...
    private final Runnable task = new Runnable() {
        ...
        public void run() {
            if (count > 1 && isAutoPlay) {
                ...
                if (onBannerChangeLisetner != null){
                    onBannerChangeLisetner.onChange();
                }
                if (currentItem == 1) {
...
//        Log.i(tag, ev.getAction() + "--" + isAutoPlay);
    ...
    public BannerBean toRealBannerBean(int position) {
        int realPosition = (position - 1) % count;
        if (realPosition < 0)
            realPosition += count;
        return imageUrls.get(realPosition);
    }
    ...
    class BannerPagerAdapter extends PagerAdapter {
        ...
        public int getItemPosition(@NonNull Object object) {
            ...
            if (((View) object).getTag() == REFRESH_TAG) {
                return POSITION_NONE;
            }
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            ...
            if (listener != null) {
                view.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        listener.OnBannerClick(toRealBannerBean(position));
                    }
                });
            }
            return view;
