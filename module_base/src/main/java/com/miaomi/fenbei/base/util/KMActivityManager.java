 ...
 * activity管理类
 */
public class KMActivityManager {

    private static Stack<WeakReference<Activity>> activityStack;
    private static KMActivityManager instance;

    private KMActivityManager() {
        activityStack = new Stack<WeakReference<Activity>>();
    }

    public static KMActivityManager get() {
        synchronized (KMActivityManager.class) {
            if (instance == null) {
                instance = new KMActivityManager();
            }
            return instance;
        }
    }
