...
public class XRecyclerView extends RecyclerView {
    ...
    private KMArrowRefreshHeader mRefreshHeader;
    ...
    private void init() {
        ...
        if (pullRefreshEnabled) {
            mRefreshHeader = new KMArrowRefreshHeader(getContext());
        }
...
//    }
    ...
    public void loadMoreComplete() {
        isLoadingData = false;
        if (mFootView instanceof LoadingMoreFooter) {
            ((LoadingMoreFooter) mFootView).setState(LoadingMoreFooter.Companion.getSTATE_COMPLETE());
        } else {
    ...
    public void setNoMore(boolean noMore){
        ...
        isNoMore = noMore;
        if (mFootView instanceof LoadingMoreFooter) {
            ((LoadingMoreFooter) mFootView).setState(isNoMore ? LoadingMoreFooter.Companion.getSTATE_NOMORE() : LoadingMoreFooter.Companion.getSTATE_COMPLETE());
        } else {
            if(footerViewCallBack != null){
    ...
    public void refresh() {
        if (pullRefreshEnabled && mLoadingListener != null) {
            mRefreshHeader.setState(KMArrowRefreshHeader.STATE_REFRESHING);
    ...
    public void setRefreshHeader(KMArrowRefreshHeader refreshHeader) {
    ...
    public void setLoadingMoreHintShowEnabled(boolean enabled) {
        loadingMoreEnabled = enabled;
        if (!enabled) {
            if (mFootView instanceof LoadingMoreFooter) {
                ((LoadingMoreFooter)mFootView).setState(LoadingMoreFooter.Companion.getSTATE_COMPLETE());
            ...
            if (mFootView instanceof LoadingMoreFooter) {
                ((LoadingMoreFooter)mFootView).setState(LoadingMoreFooter.Companion.getSTATE_SHOW_HINT());
            }
        }
    }
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 2
                            ...
                            && !isNoMore
                            && status < KMArrowRefreshHeader.STATE_REFRESHING)
            {
                isLoadingData = true;
                if (mFootView instanceof LoadingMoreFooter) {
                    ((LoadingMoreFooter) mFootView).setState(LoadingMoreFooter.Companion.getSTATE_LOADING());
                } else {
                    if(footerViewCallBack != null){
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled && appbarState == AppBarStateChangeListener.State.EXPANDED) {
                    ...
                    if (mRefreshHeader.getVisibleHeight() > 0 && mRefreshHeader.getState() < KMArrowRefreshHeader.STATE_REFRESHING) {
                        return false;
                    }
                }
                break;
