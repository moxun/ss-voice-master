...
public class GoodNumberAnimLinearLayout extends LinearLayout implements Animator.AnimatorListener {
    ...
    public GoodNumberAnimLinearLayout(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public GoodNumberAnimLinearLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//        scaleY.setInterpolator(bounceInterpolator);
        ...
        // 预先往里面塞几个数字
        for (int i = 0; i <= 8; i++) {
            GoodNumberAnimView numberView = new GoodNumberAnimView(context);
            addView(numberView);
    ...
    public void reset() {
        for (int i = 0; i < getChildCount(); i++) {
            GoodNumberAnimView giftNumberView = (GoodNumberAnimView) getChildAt(i);
...
//        }
    ...
    private void startScrollAni(LuckGiftNumbBean giftNumbBean) {
        ...
        for (int i = 1; i < arrStr.length; i++) {
            ...
            if (!anArrStr.isEmpty()) {
                GoodNumberAnimView numberView = (GoodNumberAnimView) getChildAt(i - 1);
