...
public class SpSearchUtils {
    ...
    private static SpSearchUtils instance = new SpSearchUtils();
     ...
     * xxx改为你想保存的sp文件名称
     */
    public static SpSearchUtils getInstance(Context context) {
        mContext = context;
        if (sp == null) {
     ...
     * @param keyword
     */
    public void home_save(String keyword) {
        // 获取搜索框信息
        SharedPreferences mysp = mContext.getSharedPreferences("search_home_history", 0);
        String old_text = mysp.getString("home_history", "");
        ...
        if (!old_text.contains(keyword + ",")) {
            ...
            myeditor.putString("home_history", builder.toString());
            myeditor.commit();
        }
    }

    public String[] home_getHistoryList() {
        // 获取搜索记录文件内容
        SharedPreferences sp = mContext.getSharedPreferences("search_home_history", 0);
        String history = sp.getString("home_history", "");
        // 用逗号分割内容返回数组
