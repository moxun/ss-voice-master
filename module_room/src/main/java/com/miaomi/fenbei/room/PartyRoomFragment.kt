...
class PartyRoomFragment : BaseRoomFragment(), MicClickListener, WordClickListener, ChatRoomUpdateViewCallBack, RecordVolumeObservable.RecordVolumeObserver, ChatRoomOnNetDelayCallBack, ChatRoomOnLiveFinishCallBack {
...
//                        loadData()
                ...
                override fun sendRedEnvelope(userInfo: UserInfo) {
                    NetService.getInstance(context!!).getDiamonds(object : Callback<DiamondsBean>() {
                        override fun onSuccess(nextPage: Int, bean: DiamondsBean, code: Int) {
                            bean.balance = bean.balance.replace(".00", "")
                            GiftManager.getInstance().accountBalance = bean.balance.toInt()
                ...
                private fun showRedEnvelopeDialog() {
                    RedEnvelopeDialog(mContext, userInfo, GiftManager.getInstance().accountBalance) {
                        NetService.getInstance(context!!).sendRedUser(it, userInfo.user_id.toString(), object : Callback<BaseBean>() {
                            override fun onSuccess(nextPage: Int, bean: BaseBean, code: Int) {
                                ToastUtil.suc(context!!, "红包已发送成功")
                                val c2CGiftBean = C2CCustomBean()
                                c2CGiftBean.msgType = C2CMsgBean.CUSTOM_CHAT_RED_ENVELOPE
                                c2CGiftBean.content = "到账${it}钻石"
                                val sendContent = Gson().toJson(c2CGiftBean)
                                MsgManager.INSTANCE.sendCustomMsg(userInfo.user_id.toString(), sendContent, object : SendMsgListener {
