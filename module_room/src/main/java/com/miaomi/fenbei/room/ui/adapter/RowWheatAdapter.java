...
public class RowWheatAdapter extends RecyclerView.Adapter<RowWheatAdapter.ItemHolder>{

    private List<RowBean.CiListBean> mList = new ArrayList<>();
    private Context mContext;
    private CheckItemListener mCheckListener;
    private int type;
    private int select;
    public RowWheatAdapter(Context mContext, CheckItemListener mCheckListener) {
        this.mCheckListener=mCheckListener;
        this.mContext = mContext;
    }

    public void setData(List<RowBean.CiListBean> list,int type){
        mList.clear();
        mList.addAll(list);
        this.type=type;
    ...
    public void onBindViewHolder(@NonNull final ItemHolder holder, final int position) {
        if (!TextUtils.isEmpty(mList.get(position).getFace())){
            ...
            holder.tabiconIv.setImageDrawable(mContext.getResources().getDrawable(R.drawable.icon_pop_shangmai_seat_guest));
        }
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mList.get(position).getUser_id()!=0&&type>0&&select==1){
                    ...
                    for (RowBean.CiListBean bean : mList){
