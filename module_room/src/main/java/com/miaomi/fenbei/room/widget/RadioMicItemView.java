...
//    private TextView micPosition;
    ...
    public RadioMicItemView(@NonNull Context context) {
        super(context);
        this.mContext = context;
        init();
    }

    public RadioMicItemView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        init();
    }

    public RadioMicItemView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(){
        View view = LayoutInflater.from(mContext).inflate(R.layout.room_layout_mic_item_view_radio,this,true);
...
//        micPosition = view.findViewById(R.id.item_tv_mic);
        ...
        cardIv=view.findViewById(R.id.iv_card);
...
//            ImgUtil.INSTANCE.loadGifOrWebp(mContext, "http://im-file-oss.gulajue.cn/im/img/gift_icon/1602082679_722.gif", mMicSeat1, Integer.MAX_VALUE);
        ...
        if (userInfo.getSeat_frame() == null || userInfo.getSeat_frame().isEmpty() || userInfo.getSeat_frame().equals("0")) {
            ...
            cardIv.setVisibility(VISIBLE);
        } else {
            cardIv.setVisibility(GONE);
...
//        }
        ...
        if (userInfo.getUser_id() <= 0) {
          ...
          if(position==1){
              ...
              ImgUtil.INSTANCE.loadCircleImg(mContext, R.drawable.room_icon_iron_powder, mIcon1, R.drawable.room_icon_iron_powder);
          }else if(position==3){
              ImgUtil.INSTANCE.loadCircleImg(mContext, R.drawable.room_icon_love_powder, mIcon1, R.drawable.room_icon_love_powder);
          }else if(position==4){
              ImgUtil.INSTANCE.loadCircleImg(mContext, R.drawable.room_icon_heart_powder, mIcon1, R.drawable.room_icon_heart_powder);
          }else{
              ImgUtil.INSTANCE.loadCircleImg(mContext, R.drawable.room_icon_passerby_by, mIcon1, R.drawable.room_icon_passerby_by);
            }
            }
        } else {
